 ...
 */
typedef struct {
    void *A; //array used to store nodes 
    unsigned int *key_pos; //position in A of the key of a node 
    unsigned int *rev_pos; //key of the node in a given position in A
    unsigned int num_of_elem; //number of nodes
    unsigned int max_size; //maximum numebr of nodes
    size_t key_size; //size of key type
    total_order_type leq; // heap total order
    void *max_order_value; // max value stored until now
    //since we don't know the value stored we need a void pointer
} binheap_type;
 ...
 * @param H is the heap from which the minimum should be extracted.
 **********************************************************************/
// required to avoid memory leaks because build_heap allocates memory on the heap
 ...
 *         value or the node do(es not belong to the H, NULL is returned.
 **********************************************************************/
