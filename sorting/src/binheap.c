...
void recursive_heapify(binheap_type *H, unsigned int node){

    unsigned int dst_node=node, child;
    //dst_node= node containing the min between node and children
    ...
    // if the minimum is not in node swap and recall heapify
    if (dst_node!=node){
...
const void *extract_min(binheap_type *H)
{
    ...
    // swapping keys between root and right-most leaf of the last level
    swap_keys(H,0,H->num_of_elem-1);
    
    // deliting the right most leaf of the last level
    H->num_of_elem--;    //simply changing this...
...
const void *decrease_key(binheap_type *H, void *node, const void *value)
{
    ...
    // while i reach the root and the value is greater than the parent one
